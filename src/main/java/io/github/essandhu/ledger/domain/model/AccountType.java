...
/**
 * The closed five-type chart of accounts (EQUITY exists so opening balances have a
 * legitimate counter-leg), carrying the debit/credit direction ({@code +1} for ASSET/EXPENSE,
 * {@code −1} for the rest) now that natural balances have arrived (M2).
 */
public enum AccountType {
     ...
     * expects: {@code natural = raw × direction(type)} (the formula ADR-0002's
     ...
     * sites): {@code natural = raw × direction}, CHECKED. Direction is ±1, so the
